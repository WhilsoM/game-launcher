export const games = [
	{
		id: 1,
		title: 'The Legend of Zelda: Breath of the Wild',
		description:
			'Откройте для себя мир Хайрула в этом эпическом приключении, полном загадок и опасностей.',
		img: 'http://surl.li/ghihzz',
		price: 35,
		category: 'open world',
	},
	{
		id: 2,
		title: 'The Witcher 3: Wild Hunt',
		description:
			'Станьте Геральтом из Ривии и отправьтесь в опасное путешествие по миру, полному монстров и магии.',
		img: 'http://surl.li/ymzxut',
		price: 20,
		category: 'old',
	},
	{
		id: 3,
		title: 'Red Dead Redemption 2',
		description:
			'Погрузитесь в атмосферу Дикого Запада и станьте частью банды Артура Моргана в этом захватывающем приключении.',
		img: 'http://surl.li/bevcfw',
		price: 17,
		category: 'open world',
	},
	{
		id: 4,
		title: 'Cyberpunk 2077',
		description:
			'Исследуйте футуристический город Найт-Сити и станьте наемником в мире высоких технологий и низкой морали.',
		img: 'http://surl.li/oilorq',
		price: 15,
		category: 'future',
	},
	{
		id: 5,
		title: 'Minecraft',
		description:
			'Создавайте и исследуйте бесконечные миры, строя все, что только можно вообразить, в этой культовой игре-песочнице.',
		img: 'http://surl.li/btixhj',
		price: 12,
		category: 'open world',
	},
	{
		id: 6,
		title: 'Stellar Blade',
		description:
			'Игроки берут на себя командование главной героиней Евой, которая решается спасти человечество от беспощадной войны, которую ведут чудовищные существа в далеком будущем .',
		img: 'http://surl.li/wrftqi',
		price: 55,
		category: 'stars',
	},
	{
		id: 7,
		title: 'The Last of Us Part II',
		description:
			'Спустя пять лет после страшного путешествия по охваченной эпидемией Америке Элли и Джоэл осели в Джексоне, штат Вайоминг. Обосновавшись в процветающей общине, они обрели желанную стабильность, несмотря на постоянную угрозу нападения зараженных и теряющих смысл жизни выживших.',
		img: 'http://surl.li/faazai',
		price: 90,
		category: 'zombie',
	},
	{
		id: 8,
		title: 'SEKIRO SHADOWS DIE TWICE',
		description:
			'жестокий приключенческий боевик от третьего лица, действие которого происходит в кровавой Японии XIV века.',
		img: 'http://surl.li/obvnbl',
		price: 45,
		category: 'ninja',
	},
	{
		id: 9,
		title: 'ELDEN RING',
		description:
			'Действие игры происходит в Междуземье — стране «за завесой тумана», которой с давних времён правила королева Марика Вечная. В Междуземье воля его правителя определяет саму реальность. ',
		img: 'http://surl.li/bzmafs',
		price: 31,
		category: 'fears',
	},
]

export const friends = [
	{
		id: 1,
		nickname: 'JohnyStephan',
		playingGame: 'Zelda',
		avatar: 'https://www.heropack.me/img/styles/survival-man.jpeg',
	},
	{
		id: 2,
		nickname: 'reginakjl',
		playingGame: 'Red Dead 2',
		avatar: 'https://www.heropack.me/img/styles/psycho-man.jpeg',
	},
	{
		id: 3,
		nickname: 'IceDragon',
		playingGame: 'The Witcher 3',
		avatar: 'https://www.heropack.me/img/styles/vampire-illustration-man.jpeg',
	},
	{
		id: 4,
		nickname: 'moonw',
		playingGame: 'Cyberpunk 2077',
		avatar: 'https://www.heropack.me/img/styles/royalty-man.jpeg',
	},
]

export const videos = [
	{
		id: 1,
		url: 'https://www.youtube.com/watch?v=wYW07LpGhYE&ab_channel=DvaPointGames',
		title: 'Cyberpank 2077',
		description:
			'Исследуйте футуристический город Найт-Сити и станьте наемником в мире высоких технологий и низкой морали.',
	},
	{
		id: 2,
		url: 'https://www.youtube.com/watch?v=MqQ3LeBgGDM&ab_channel=TheWitcher',
		title: 'The Witcher 3: Wild Hunt',
		description:
			'Мир охвачен войной. Армия империи Нильфгаард стирает все на своем пути, стремясь добраться до самого сердца Северных королевств.',
	},

	{
		id: 3,
		url: 'https://youtu.be/uDMWRRAEogs?si=qt-4pBR0IvbSOZco&t=22',
		title: 'The Legend of Zelda: Breath of the Wild',
		description:
			'10 000 лет спустя, вернувшись в средневековое государство, королевство Хайрул заново открыло древнюю технологию, оставленную их предками, посредством раскопок. Они узнали, что Бедствие Ганон вернётся, чтобы остановить его.',
	},
	{
		id: 4,
		url: 'https://youtu.be/DSznLWimMlU?si=fnolofBOinsKrf_d&t=4',
		title: 'Stellar Blade',
		description:
			'Действие происходит в недалеком будущем на Земле, опустошенной загадочными врагами, изгнавшими человечество с родной планеты.',
	},
	{
		id: 5,
		url: 'https://youtu.be/rXMX4YJ7Lks?si=EDMWptHKDMI8HXZc&t=2',
		title: 'Sekiro: Shadows Die Twice',
		description:
			'изуродованным воином-изгоем, которого спасли от верной смерти. Поклявшись защищать молодого господина, являющегося наследником древнего рода, вы наживаете много опасных противников, среди которых — безжалостный клан Асина.',
	},
	{
		id: 6,
		url: 'https://youtu.be/Ye3st9z6jQY?si=gusuXU6w9hS6T_yX&t=3',
		title: 'The Last of Us Part II',
		description:
			'Человеческая цивилизация уничтожена эпидемией, вызванной мутацией гриба-паразита, вакцин нет, иммунитета нет. Уцелевшие люди обитают в руинах и умудряются враждовать друг с другом даже в таких условиях.',
	},
	{
		id: 7,
		url: 'https://youtu.be/E3Huy2cdih0?si=ux5meSy9pr7PRbRY&t=4',
		title: 'ELDEN RING',
		description:
			'Междуземье — стране «за завесой тумана», которой с давних времён правила королева Марика Вечная. В Междуземье воля его правителя определяет саму реальность.',
	},
]

// мб сделать на странице библиотеки "имеющиеся" игры
export const availableGames = []
